...
# Add "lineage" back into lineages
samp_spol <- dplyr::select(data, sample, spoligotype)
non_samp_spol <- dplyr::select(data, -sample, -spoligotype)
non_samp_spol <- data.frame(apply(non_samp_spol, 2, function(x){
  paste0("lineage", x)
  }))
data <- cbind(samp_spol, non_samp_spol)

# Make a freq table
lv1_tab <- tab2df(select(data, spoligotype, lin_level_1))

spol <- row.names(lv1_prob)
col_nms <- colnames(lv1_prob)

for(col in seq(ncol(lv1_prob))){
  sub <- subset(lv1_prob[col], lv1_prob[col] > 0)
  df <- data.frame(spol = row.names(sub), sub)
  print(df)
}
