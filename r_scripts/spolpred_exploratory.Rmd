...
lv1_df <- merge(lv1_df, sit_fam_spol, 
                by = "spoligotype", 
                all.x = T, sort = F)
...
```{r spol-predictions-loop, echo = T, warning=F}
...
#####
# TEST with just first two levels
levels <- c("lin_level_1", "lin_level_2")
#####
...
for(level in levels){
  ...
  lv_tab <- tab2df(dplyr::select(spol_data, spoligotype, level))
  ...
  lv_df <- merge(lv_df, sit_fam_spol, 
                  by = "spoligotype", 
                  all.x = T, sort = F)
