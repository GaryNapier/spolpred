...
lv1_col_prob$spol <- row.names(lv1_col_prob)
...
for(lin in names(lv1_list)){
  ...
  # Merge in the freqs
  lv1_list[[lin]] <- setNames(merge(lv1_list[[lin]], lv1_tab[, c("spol", lin)], 
        by = "spol", 
        sort = F), 
        c("spol", lin, "freq"))
  
  # Merge in the freq as a proportion of the total n samps per lin
  lv1_list[[lin]] <- setNames(merge(lv1_list[[lin]], lv1_col_prob[, c("spol", lin)],
      by = "spol",
      sort = F), 
      c("spol", "prob", "freq", "col_prob"))
