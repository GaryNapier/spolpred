...
```{r spol-predictions, echo = T, warning=F}
...
spol_data <- cbind(samp_spol, non_samp_spol)

head(spol_data)

# Make a freq table
lv1_tab <- tab2df(select(spol_data, spoligotype, lin_level_1))
...
lv1_tab$spol <- row.names(lv1_tab)
lv1_prob$spol <- row.names(lv1_prob)

# Pull out the non-zeros for each lineage
lv1_list <- list()
for(col in col_nms){
  
  df <- lv1_prob[, c("spol", col)]
...
for(lin in names(lv1_list)){
  # print(lv1_list[[lin]])
  print(merge(lv1_list[[lin]], lv1_tab[, c("spol", lin)], 
        by = "spol", 
        sort = F))
}
