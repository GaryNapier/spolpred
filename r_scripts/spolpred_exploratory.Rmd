...
lv1Summary <- data %>% dplyr::count(spoligotype, lin_level_1)
lv2Summary <- data %>% dplyr::count(spoligotype, lin_level_2)
lv3Summary <- data %>% dplyr::count(spoligotype, lin_level_3)
lv4Summary <- data %>% dplyr::count(spoligotype, lin_level_4)
lv5Summary <- data %>% dplyr::count(spoligotype, lin_level_5)
...
# fruit L1  L2    L3
# mangoes 90 100 20000
# bananas 10 900 80000

# fruit       L1      L2    L3
# mangoes 0.004458 0.00495 0.991
# bananas 0.000124 0.01112 0.989

# table(dummy)
# prop.table(table(dummy))
# row_probs(as.data.frame.matrix(table(dummy)))
# 
# prop.table(num_cols(x))
# 
# col_sums <- colSums(num_cols(x))
# 
# t(apply(row_probs(num_cols(x)), 1, function(x){x/col_sums}))

# x_num <- num_cols(x)
# 
# col_wise <- col_probs(x_num)
# 
# rowsums_colwise <- rowSums(col_wise)
# 
# mat <- matrix(nrow = nrow(x_num), ncol = ncol(x_num))
# for(row in seq(nrow(x_num))){
#   for(col in seq(ncol(x_num[row, ])))
#     mat[row, col] <- x_num[row,col]/rowsums_colwise[row]
# }
# mat
...
L2 <- subset(dummy_bananas, lineage == "L2")
L3 <- subset(dummy_bananas, lineage == "L3")

L2 <- data.frame(spol = rep(L2$spol, 10), lineage = rep(L2$lineage, 10))
L3 <- data.frame(spol = rep(L3$spol, 10), lineage = rep(L3$lineage, 10))

dummy_bananas <- rbind(dummy_bananas, L2, L3)


dummy_bananas_freqs <- as.data.frame.matrix(table(dummy_bananas))

row_probs(dummy_bananas_freqs)

col_probs(dummy_bananas_freqs)
...
n_samps <- round_any(floor(min_samp_sz*0.8), 10)
