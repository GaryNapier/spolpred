...
The most frequent spoligotype family in our data belongs to Beijing (8,056), followed by T (6,000) and LAM (4,229). The most common geographical regions in which our samples are found were Northern Europe (5,018), Southern Africa (4,488) and Southeast Asia (3,650). **Figure \@ref(fig:region-family-plot)** shows the frequency of samples gathered in a geographical region versus the spoligotype family. While many samples occur in their expected geographical region, there is much variation as to where in the world a spoligotype family is found. The geographical distribution of our data with respect to spoligotype family perhaps reflects samples being taken as part of transmission studies or other clinically relevant investigation. 
...
```{r region-family-plot, echo = F, warning=F, fig.cap="Heatmap of number of samples by spoligotype family and region of sample collection"}
...
knitr::kable(fmt(lin_fam_freq_tab), row.names = T, caption = cap, format = "pipe")
