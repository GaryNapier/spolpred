...
# Kepp only spols with at least 5 samps
...
spol_n_sublin <- reshape2::dcast(lin_fam_geo_df, spoligotype ~ 'n_sublin', 
                 value.var = 'lineage', fun.aggregate = len_uniq)


# spol_lin <- select(lin_fam_geo_df, spoligotype, lineage)

spol_lin_n <- reshape2::dcast(lin_fam_geo_df, spoligotype + lineage ~ 'n', 
                 value.var = 'sample', fun.aggregate = len_uniq)


spol_lin_n_split <- split(spol_lin_n, spol_lin_n$spoligotype)

x <- head(spol_lin_n_split[unlist(lapply(spol_lin_n_split, function(x){nrow(x) > 1}))], 10)
...
# Wrangle for plot
# Normalise across col (Have to transpose for some reason. Stupid!)
...
```{r spol_freq-hist, echo = F, warning=F, message=F}
